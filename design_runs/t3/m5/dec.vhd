...
-- The decoder for BCH code (31,16), t=3
...
ARCHITECTURE dpdbma OF dpdbm IS
	...
	SIGNAL m0in, m1in, m2in, m3in, m4in: BIT_VECTOR(0 TO m-1);
  BEGIN
	aux(0)<= ddin(0) XOR ddin(2);
	aux(1)<= ddin(1) XOR ddin(3);
	aux(2)<= ddin(2) XOR ddin(4);
	aux(3)<= ddin(3) XOR aux(0);
	m0in<= ddin(0) & ddin(1) & ddin(2) & ddin(3) & ddin(4);
	m1in<= ddin(1) & ddin(2) & ddin(3) & ddin(4) & aux(0);
	m2in<= ddin(2) & ddin(3) & ddin(4) & aux(0) & aux(1);
	m3in<= ddin(3) & ddin(4) & aux(0) & aux(1) & aux(2);
	m4in<= ddin(4) & aux(0) & aux(1) & aux(2) & aux(3);
	m0:  dsdbm
...
ARCHITECTURE dpma OF dpm IS
	...
	SIGNAL c0, c1, c2, c3, c4: BIT_VECTOR(0 TO m-1);
  BEGIN
	b(0 TO m-1)<= din1;
	b(5)<= b(1) XOR b(4);
	b(6)<= b(0) XOR b(3);
	b(7)<= b(4) XOR b(2);
	b(8)<= b(3) XOR b(5);
	c0<= b(0) & b(4) & b(3) & b(2) & b(5);
	m0: dsdbm
		PORT MAP (din2, c0, dout(0));
	c1<= b(1) & b(0) & b(4) & b(3) & b(2);
	m1: dsdbm
		PORT MAP (din2, c1, dout(1));
	c2<= b(2) & b(5) & b(6) & b(7) & b(8);
	m2: dsdbm
		PORT MAP (din2, c2, dout(2));
	c3<= b(3) & b(2) & b(5) & b(6) & b(7);
	m3: dsdbm
		PORT MAP (din2, c3, dout(3));
	c4<= b(4) & b(3) & b(2) & b(5) & b(6);
	m4: dsdbm
...
BEGIN
	...
	dout(1)<= din(3);
	dout(2)<= din(4);
	dout(3)<= din(0) XOR din(2);
	dout(4)<= din(1) XOR din(3);
END dmlia;
...
ARCHITECTURE dsqa OF dsq IS
	SIGNAL dxor: BIT_VECTOR(0 TO 4);
  BEGIN
	dxor(0 TO m-1)<= din;
		-- optimization saving = 0 XOR gates
	dout(0)<= dxor(0) XOR dxor(4);
	dout(1)<= dxor(3);
	dout(2)<= dxor(1) XOR dxor(4);
	dout(3)<= dxor(3) XOR dxor(4);
	dout(4)<= dxor(2);
	-- no. XOR gates = 3
...
ARCHITECTURE dcheqa OF dcheq IS
  ...
  BEGIN
	...
	dout<= NOT (eq(0) OR eq(1) OR eq(2) OR eq(3) OR eq(4));
END dcheqa;
...
ARCHITECTURE dsyn1a OF dsyn1 IS
  ...
  BEGIN
	syn(5)<= syn(1) XOR syn(4); -- 2
	syn(6)<= syn(3) XOR syn(4); -- 2
	syn(7)<= syn(0) XOR syn(4); -- 1
		-- Saving due to optimization = 5
	...
	dout2(3)<= syn(6);
	dout2(4)<= syn(2);
	dout4(0)<= syn(2) XOR syn(7);
	dout4(1)<= syn(6);
	dout4(2)<= syn(2) XOR syn(3);
	dout4(3)<= syn(2) XOR syn(6);
	dout4(4)<= syn(5);
  PROCESS BEGIN
	WAIT UNTIL clk'EVENT AND clk='1';
	IF pe='1' THEN
...
ARCHITECTURE dsyn3a OF dsyn3 IS
  ...
  PROCESS BEGIN
	...
	syn(0)<= din XOR syn(2);
	syn(1)<= syn(3);
	syn(2)<= syn(2) XOR syn(4);
	syn(3)<= syn(0) XOR syn(3);
	syn(4)<= syn(1) XOR syn(4);
	END IF;
  END PROCESS;
END dsyn3a;
...
ARCHITECTURE dsyn5a OF dsyn5 IS
  ...
  PROCESS BEGIN
	...
	IF pe='1' THEN
	  ...
	  syn(1 TO 4)<= "0000";
	ELSE
	syn(0)<= din XOR syn(0) XOR syn(3);
	syn(1)<= syn(1) XOR syn(4);
	syn(2)<= syn(0) XOR syn(2) XOR syn(3);
	syn(3)<= syn(1) XOR syn(3) XOR syn(4);
	syn(4)<= syn(2) XOR syn(4);
	END IF;
  END PROCESS;
END dsyn5a;
...
ARCHITECTURE dch1a OF dch1 IS
	...
	SIGNAL ch: BIT_VECTOR(0 TO 4); -- ch registers and optimization 
  BEGIN
	dout<= ch(0 TO m-1);
	chin(0)<= ch(4);
	chin(1)<= ch(0);
	chin(2)<= ch(1) XOR ch(4);
	chin(3)<= ch(2);
	chin(4)<= ch(3);
  PROCESS BEGIN
	WAIT UNTIL clk'EVENT AND clk='1';
...
ARCHITECTURE dch3a OF dch3 IS
  ...
  BEGIN
	...
	chin(0)<= ch(2);
	chin(1)<= ch(3);
	chin(2)<= ch(2) XOR ch(4);
	chin(3)<= ch(0) XOR ch(3);
	chin(4)<= ch(1) XOR ch(4);
  PROCESS BEGIN
	WAIT UNTIL clk'EVENT AND clk='1';
	IF pe='1' THEN
	  ch(0 TO m-1)<= din;
...
ARCHITECTURE dcounta OF dcount IS
  ...
  BEGIN
	...
	clastb<= NOT cb(0) AND cb(1) AND NOT cb(2) AND cb(3);  -- cb= 10
	clasta<= NOT ca(0) AND NOT ca(1);  -- ca= 0
	clast<= clasta AND clastb; -- count= 30
	chpe1b<= NOT cb(0) AND cb(1) AND NOT cb(2) AND NOT cb(3);  -- cb= 2
	chpe1a<= ca(0) AND NOT ca(1);  -- ca= 1
	chpe1<= chpe1a AND chpe1b; -- count= 7
	vdout1Rb<= NOT cb(0) AND NOT cb(1) AND NOT cb(2) AND cb(3);  -- cb= 8
	vdout1Ra<= NOT ca(0) AND NOT ca(1);  -- ca= 0
	vdout1R<= vdout1Ra AND vdout1Rb; -- count= 24
	vdout1Sb<= NOT cb(0) AND cb(1) AND NOT cb(2) AND NOT cb(3);  -- cb= 2
	vdout1Sa<= NOT ca(0) AND ca(1);  -- ca= 2
	vdout1S<= vdout1Sa AND vdout1Sb; -- count= 8
	bufRb<= cb(0) AND NOT cb(1) AND cb(2) AND NOT cb(3);  -- cb= 5
	bufRa<= NOT ca(0) AND NOT ca(1);  -- ca= 0
	bufR<= bufRa AND bufRb; -- count= 15
	cb0<= NOT cb(0) AND NOT cb(1) AND NOT cb(2) AND NOT cb(3);  -- cb= 0
	lCe<= ca0;

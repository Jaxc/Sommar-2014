...
ARCHITECTURE dcheqa OF dcheq IS
  ...
  BEGIN
	...
	eq(4)<= din1(4) XOR din2(4) XOR din3(4);
	eq(5)<= din1(5) XOR din2(5) XOR din3(5);
...
ARCHITECTURE dcaa OF dca IS
  ...
  BEGIN
	...
	cLast<= ca(0) AND ca(1) AND ca(2);  -- ca= 7
...
PORT (clk, reset, drnzero: IN BIT;
	bsel, bufCe, bufkCe, chpe, msmpe, snce, synpe, vdout, vdout1,c0first, cce, caLast1, cbBeg, dringPe: OUT BIT);
...
ARCHITECTURE dcounta OF dcount IS
  ...
  BEGIN
	...
	clastb<= cb(0) AND cb(1) AND cb(2);  -- cb= 7
	clasta<= NOT ca(0) AND ca(1) AND ca(2);  -- ca= 6
	clast<= clasta AND clastb; -- count= 62
	chpe1b<= NOT cb(0) AND cb(1) AND NOT cb(2);  -- cb= 2
	chpe1a<= NOT ca(0) AND ca(1) AND ca(2);  -- ca= 6
	chpe1<= chpe1a AND chpe1b; -- count= 22
	vdout1Rb<= NOT cb(0) AND NOT cb(1) AND NOT cb(2);  -- cb= 0
	vdout1Ra<= ca(0) AND NOT ca(1) AND ca(2);  -- ca= 5
	vdout1R<= vdout1Ra AND vdout1Rb; -- count= 5
	vdout1Sb<= NOT cb(0) AND cb(1) AND NOT cb(2);  -- cb= 2
	vdout1Sa<= ca(0) AND ca(1) AND ca(2);  -- ca= 7
	vdout1S<= vdout1Sa AND vdout1Sb; -- count= 23
	bufRb<= cb(0) AND NOT cb(1) AND cb(2);  -- cb= 5
	bufRa<= NOT ca(0) AND NOT ca(1) AND ca(2);  -- ca= 4
	bufR<= bufRa AND bufRb; -- count= 44
	cb0<= NOT cb(0) AND NOT cb(1) AND NOT cb(2);  -- cb= 0
	lCe<= caLast AND NOT cb0;
	caLast1<= caLast;
	cceR<= ca(0) AND NOT ca(1) AND ca(2);  -- ca= 5
	...
	dringPe2<= NOT ca(0) AND NOT ca(1) AND ca(2);  -- ca= 4
	msmpe1<= ca(0) AND NOT ca(1) AND NOT ca(2);  -- ca= 1
	ca0<= NOT ca(0) AND NOT ca(1) AND NOT ca(2);  -- ca= 0
	caNextLast<= NOT ca(0) AND ca(1) AND ca(2);  -- ca= 6
	bufCe<= '1';
	cei1<= '1';
END dcounta;	
...
ARCHITECTURE deca OF dec IS
  ...
  BEGIN
	...
	ms4: dxort
		PORT MAP (sn0en(4), sn1en(4), sn2en(4), sn3en(4), cs(4));
	ms5: dxort
		PORT MAP (sn0en(5), sn1en(5), sn2en(5), sn3en(5), cs(5));
	c2: dshr
	...
	count: dcount
		PORT MAP (clk, reset, drnzero, bsel, bufCe, bufkCe, chpe, msmPe, snce, synpe, vdout, vdout1, c0first, cce, caLast, cbBeg, dringPe);
